# LAB 1 - AI and ML

## 1. Create Azure Machine Learning
![Azure Machine Learning](img/1.png)

## 2. Azure Machine Learning Studio
![Azure Machine Learning Studio](img/2.png)

## 3. Create a Training Pipeline
![Create a Training Pipeline](img/3.png)

## 4. Create a New Experiment
![Create a New Experiment](img/4.png)

## 5. Create Compute Instance
![Create Compute Instance](img/5.png)

# 6. Pipeline Created Successfully
![Pipeline Created Successfully](img/6.png)

# 7. Job Successful
![Pipeline Created Successfully](img/7.png)

# 8. Result Dataset
![Pipeline Created Successfully](img/8.png)
